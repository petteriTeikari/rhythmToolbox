function cosOut = cosinor(t,y,w,alpha)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%   Finds individual confidence intervals for Amplitude and Acrophase 
%   and plots a polar plot representation of the cosinor fit.
%
% Input:
%   t - time series
...
%     3/25/11        Petteri Teikari    Revisions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% flag to determine whether the output is printed on command window
displayResults = 0;
plotResults    = 0;
...
%% II. Confidence Limits for Single Cosinor
    ...
    [CI_Amp_min, CI_Amp_max, CI_phi_min, CI_phi_max, param] = cosinor_CIcalc(X,T,Z,beta,gamma,n,sigma,Amp,phi,alpha); 
...
%% OUTPUT Structure
    ...
    % Fit, for plotting    
    cosOut.fit   = f;    % the fit that can be plotted

    % COSINOR Parameters
    cosOut.M    = M;    % M   - Mesor, the average cylce value
    cosOut.Amp  = Amp;  % Amp - Amplitude, half the distance between peaks of the fitted  waveform
    cosOut.phi  = phi;  % phi - Acrophase, time point in the cycle of highest amplitude (in radians)
    cosOut.RSS  = RSS;  % RSS - Residual Sum of Squares, a measure of the deviation of the cosinor fit from the original waveform
                        
    % confidence intervals
    cosOut.CI.M         = CI_M;         % Mesor limits
    cosOut.CI.AmpMin    = CI_Amp_min;   % Min. amplitude
    cosOut.CI.AmpMax    = CI_Amp_max;   % Max. amplitude
    cosOut.CI.PhiMin    = CI_phi_min;   % Min. Acrophase
    cosOut.CI.PhiMax    = CI_phi_max;   % Max. Acrophase
    cosOut.CI.p_3a      = p_3a;         % p of zero-amplitude test
    
    
%% PRINT and PLOT (if needed / wanted)
    
    %% If display to command window is wanted
    if displayResults == 1
    ...
    %% If plotting is wanted
    if plotResults == 1
        
        scrsz = get(0,'ScreenSize');
        
        %Plot orginal data and cosine fit
        fig(1) = figure('name','Cosinor Analysis: Original data and fitted function', 'position', [0.01*scrsz(3) 0.52*scrsz(4) 0.49*scrsz(3) 0.45*scrsz(4)]);
        ...
        fig(2) = figure('name','Rhythm Parameter Estimates with Joint Confidence Region', 'position', [0.51*scrsz(3) 0.52*scrsz(4) 0.45*scrsz(3) 0.45*scrsz(4)]);

            plot(param.gamma_s,param.beta_s1,'linewidth', 2); hold on;
            plot(param.gamma_s,param.beta_s2,'linewidth', 2)
