function [CI_Amp_min, CI_Amp_max, CI_phi_min, CI_phi_max, param] = cosinor_CIcalc(X,T,Z,beta,gamma,n,sigma,Amp,phi,alpha)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Confidence Interval Calculations
%
% Description: 
%   Subfuction of 'cosinor.m'. 
%
...
%     3/25/11        Petteri Teikari    Minor Revisions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
F_distr = finv(1-alpha,2,n-3);
    ...
    if (range(gamma_s) >= max(gamma_s)) && ((range(beta_s1) >= max(beta_s1)) || (range(beta_s2) >= max(beta_s2)))
        ...
        theta = cat(2,[atan(abs(gamma_s./beta_s1))], [atan(abs(gamma_s./beta_s2))]);
            ...
            CIphi = zeros(length(c),1); % preallocate memory
            for ii = 1:length(c);
                if (c(ii) == 4 || c(ii) == 3)
                    CIphi(ii) = -theta(ii);
                    c(ii) = 1;
    ...
    % Extra output structure, needed for plotting
    param.gamma_s = gamma_s;
    param.beta_s1 = beta_s1;
    param.beta_s2 = beta_s2;
